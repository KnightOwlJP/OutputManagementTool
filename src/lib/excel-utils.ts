...
export function generateProcessDataXLSX(processes: Process[], projectName?: string): Blob {
  ...
  const summaryData = [
    ...
    [''],
    ['ステータス別'],
    ['完了', processes.filter((p) => p.status === '完了' || p.status === 'completed').length],
    ['進行中', processes.filter((p) => p.status === '進行中' || p.status === 'in_progress').length],
    ['未着手', processes.filter((p) => p.status === '未着手' || p.status === 'not_started').length],
    ['保留', processes.filter((p) => p.status === '保留' || p.status === 'on_hold').length],
    ['中止', processes.filter((p) => p.status === '中止' || p.status === 'cancelled').length],
    [''],

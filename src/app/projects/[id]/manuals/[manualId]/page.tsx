...
export default function ManualEditorPage() {
  ...
  const loadManual = async () => {
    ...
    try {
      // マニュアル情報を取得
      const manuals = await window.electronAPI.manual.list(projectId);
      const currentManual = manuals.find(m => m.id === manualId);
  ...
  const handleSyncWithProcesses = async () => {
    ...
    try {
      await window.electronAPI.manual.syncFromProcesses(manual.id);
      await loadManual();
      alert('工程表との同期が完了しました');
    } catch (error) {

...
### 1.1 目的
工程表・BPMNフローチャート・マニュアルを三位一体で統合管理し、階層的な工程管理とバージョン管理を実現するデスクトップアプリケーションの開発。

**コア価値**:
- **BPMN ⇔ 工程表**: 双方向リアルタイム同期
- **工程表 → マニュアル**: 小工程・詳細工程からマニュアル自動生成
...
- **大工程**: 部署間の区切り（部署単位でのタスク分割）
  - 例: 営業部門、開発部門、経理部門
- **中工程**: 部署内の作業実行者の区切り（担当者・役割単位でのタスク分割）
  - 例: プロジェクトマネージャー、設計担当、実装担当
- **小工程**: 帳票の区切り（帳票・ドキュメント単位でのタスク分割）
  - 例: 申請書作成、設計書作成、報告書作成
- **詳細**: 小工程内の具体的な作業手順・ステップレベル（マニュアル記述対象）
  - 例: 申請書の項目入力、承認者選択、提出
...
#### 3.3.2 階層ナビゲーション
- **ツリー表示**: 階層構造の一覧表示
  - 部署 → 作業実行者 → 帳票 → 作業手順 の順で表示
- **ドリルダウン**: 上位階層から下位階層への遷移
- **ブレッドクラム**: 現在位置の表示
  - 例: `営業部門 > 営業担当 > 見積書作成 > 項目入力`
- **親子関係の可視化**: 階層間の関連を明示
- **部署/担当者/帳票フィルタ**: 各階層レベルでのフィルタリング機能

#### 3.3.3 BPMNとの連携
- **サブプロセス活用**: 階層の詳細をサブプロセスで表現
- **コールアクティビティ**: 別ダイアグラムへの参照
- **階層間の同期**: 工程表の階層とBPMNフローの階層を連動
...
- **検索機能**: 全文検索・タグ検索
- **カテゴリ管理**: マニュアルの分類・整理
- **工程との紐付け**: 小工程・詳細工程とマニュアルセクションの関連管理

#### 3.6.2 工程表からのマニュアルアウトライン自動生成 ⭐ Phase 6実装範囲
- **対象工程**: 
  - **小工程**: マニュアルの章・節レベル
  - **詳細工程**: マニュアルの具体的な手順レベル
  - 小工程・詳細工程の内容を基にマニュアル構造を自動構成
- **自動生成フロー（Phase 6）**:
  ```
  工程表（小工程・詳細工程）
    ↓
  アウトライン構造生成（章立てのみ）
    ↓
  階層構造の反映（大工程→章、中工程→節、小工程→項、詳細→手順）
    ↓
  工程との紐付けID保存
    ↓
  マニュアルアウトライン完成
  ```
- **セクション管理**:
  - 大工程・中工程・小工程・詳細の4段階で自動セクション構成
  - 小工程以降（小工程・詳細）を重点的にマニュアル化
  - セクション階層は工程階層と完全同期
  - 例: 部署（大工程）→章、作業実行者（中工程）→節、帳票（小工程）→項、手順（詳細）→具体的な手順
- **テンプレート選択**: 
  - 業務マニュアル形式
  - 操作手順書形式
  - フローチャート説明書形式
...
- **Phase 6コンテンツ生成範囲**:
  ...
  - ✅ 工程IDとの紐付け
  - ✅ プレースホルダーテキスト
  - ❌ 詳細な本文生成（Phase 7で生成AI連携）

> **🔮 Phase 7実装予定: 生成AIによる詳細コンテンツ自動生成**
...
interface ManualSection {
  ...
  order: number;           // 順序
  level: 'large' | 'medium' | 'small'; // セクション粒度（大・中・小の任意）
  heading: string;         // 見出し
  content: string;         // 内容
  processId?: string;      // 関連工程ID（大・中・小・詳細のいずれか）
  processLevel?: 'large' | 'medium' | 'small' | 'detail'; // 関連工程レベル
  bpmnElementId?: string;  // 関連BPMN要素ID
  parentSectionId?: string; // 親セクションID（階層構造）
  images?: string[];       // 画像パス
}
```
